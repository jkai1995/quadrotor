...
{		 					 
  ...
  TIM_Cmd(TIM5,DISABLE ); 	//使能定时器5
	TIM_ITConfig(TIM5,TIM_IT_CC2,DISABLE);//禁止中断
...
{		 					 
	...
	GPIO_PinAFConfig(GPIOE,GPIO_PinSource5,GPIO_AF_TIM9); //GPIOE5 E6复用为定时器9
	GPIO_PinAFConfig(GPIOE,GPIO_PinSource6,GPIO_AF_TIM9); //GPIOE5 E6复用为定时器9
...
{		 					 
	...
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource14,GPIO_AF_TIM12); //GPIOB14 B15复用为定时器9
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource15,GPIO_AF_TIM12); //GPIOB14 B15复用为定时器9
...
ST_RunTime_us_TypeDef run_time_us; //计量程序运行时间
