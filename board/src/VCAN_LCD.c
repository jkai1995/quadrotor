...
void LCD_num_BCHQL(Site_t site, int32 num, uint8 max_num_bit, uint16 Color, uint16 bkColor)
{
    if(num>=0)
    {
        LCD_num_BC(site, (uint32)num,  max_num_bit+1,  Color,  bkColor);
    }
    else
    {
        num=-num;
        ...
        site.x+=4;
        LCD_num_BC(site, (uint32)num,  max_num_bit,  Color,  bkColor);
    }
}
/*!
 *  @brief      灰度图像显示
...
/////////////////////////////////////////////////////////////////////////////////////////////////////
void LCD_curve_hql(Site_t start,int8 *cline,uint16 Color,uint8 stav,uint8 endv)
{
    int v;
    Site_t site;
    for(v=stav;v>=endv;v--)
